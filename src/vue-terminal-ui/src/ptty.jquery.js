        ...
        this.each(function () {
                ...
                _public.native_commands = function () {
                    ...
                    _public.register('command', {
                        name: 'help',
                        method: function (cmd) {
                            if (cmd['help'].length > 1) {
                                var command = cmd['help'][1]
                                if (typeof commands[command] !== 'undefined') {
                                    cmd.out = '<b>' + command + '</b> - ';
                                    if (commands[command].help !== '') {
                                        cmd.out += commands[command].help + "\n";
                                        cmd.out += "<div style='color: #677; margin: 8px 0;'><b>params:</b> "+ command.toUpperCase() +" "+_public.helpers[command.toUpperCase()].params+"</div>"
                                    } else {
                ...
                _public.tokenizeAlwarys = function (command) {
                    var out = [];
                    var cmd = $.trim(command).split(/\s+/);
                    var option = false, value = false, quote_type = false, quote_open = false,
                        first_char = false, last_char = false, before_last = false;
                    for (var i = 0; i < cmd.length; i++) {
                        ...
                        if (last_char === quote_type && quote_open === true && before_last + last_char !== '\\' + quote_type) { // 引号闭合
                            value += ' ' + cmd[i];
                            quote_open = false;
                            // Trim & Strip any ecaping slashes
                            value = $.trim(value.substring(1).slice(0, -1).replace(/\\(.)/mg, "$1"));
                            console.log('value', value)
                        } else if (first_char === '"' && quote_open === false) { // 双引号开始
                            quote_type = '"';
                            quote_open = true;
                            if (last_char === quote_type && before_last + last_char !== '\\' + quote_type) {
                                quote_open = false
                                value = $.trim(cmd[i].substring(1).slice(0, -1));
                            } else {
                                value = cmd[i];
                            }
                        } else if (first_char === "'" && quote_open === false) { // 单引号开始
                            quote_type = "'";
                            quote_open = true;
                            if (last_char === quote_type && before_last + last_char !== '\\' + quote_type) {
                                quote_open = false
                ...
                input.after("<div class='command_tips'></div>");
                var commandTip = el.find('.prompt .command_tips');
                ...
                var cmd_start = function (direct_cmd) {
                    ...
                    if (!cmd || cmd == '') {
                        ...
                        var cmd_names = cmd.split(/\s+/);
                        cmd_name = cmd_names[0].toUpperCase();
                        if (['CONFIG', 'COMMAND', 'CLUSTER', 'CLIENT'].includes(cmd_name) && !commands[cmd_name] && cmd_names[1]) {
                            cmd_name += " " + cmd_names[1].toUpperCase()
                        }
                        cmd_name = cmd_name.toLowerCase();
                    }
                    if (typeof commands[cmd_name] !== 'undefined') {
                        cmd_obj = _public.tokenize(cmd); //get command token list
                        cmd_obj.str = JSON.stringify(_public.tokenizeAlwarys(cmd));
                        // console.log('tokenizeAlwarys', cmd_obj.str)
                    } else {
                        if (!quiet) {
                ...
                input.keyup(function () {
                    var inputVal = $.trim(input.text())
                    var commands = inputVal.split(" ")
                    var command = commands[0].toUpperCase();
                    var commandInfo = _public.helpers[command];
                    if (commandInfo) {
                        commandTip.html("<div style='color: #677'>" + command + " " + commandInfo["params"] + "</div>")
                    } else if (commands.length > 1 && ['CONFIG', 'COMMAND', 'CLUSTER', 'CLIENT'].includes(command)) {
                        command = (commands[0] + " " + commands[1]).toUpperCase();
                        commandInfo = _public.helpers[command]
                        if (commandInfo) {
                            commandTip.html("<div style='color: #677'>" + command + " " + commandInfo["params"] + "</div>")
                        } else {
                            commandTip.html("")
                        }
                    } else if (commands.length === 1 && command.length > 0) {
                        var sameCommands = [];
                        for (var i in _public.helpers) {
                            if (i.indexOf(command) === 0) {
                                sameCommands.push(i)
                            }
                        }
                        commandTip.html("<div style='color: #677'>" + sameCommands.join(" ") + "</div>")
                    } else {
                        commandTip.html("")
                    }
                })
